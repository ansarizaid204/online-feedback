...
if(isset($_POST['submit']))
{
        ...
        $shift = $_POST['shift'];
        $status = $_POST['status'];
$query3=mysqli_query($conn,"UPDATE student SET name='$name', stud_id='$stud_id', password='$password', email='$email', mob='$mob', enr_no='$enr_no', gender='$gender', dept_id='$dept_id', sem_id='$sem_id', shift='$shift', s_id='$status' WHERE id='$id'");
if($query3)
{
header("location:student.php");
}
}
$query1=mysqli_query($conn,"SELECT * FROM student WHERE id='$id'");
...
<table>
    <tr>
        <td>Name</td>
        <td><input type="text" name="name" placeholder="Name" class="textinput1" required autofocus value="<?php echo $query2['name']; ?>" ></td>
    </tr>
    <tr>
        <td>Student ID</td>
        <td><input type="text" name="stud_id" placeholder="Student ID" class="textinput1"  value="<?php echo $query2['stud_id']; ?>" onkeypress="return isNumber(event)" ></td>
    </tr>
    <tr>
        <td>Password</td>
        <td><input type="password" name="password" placeholder="password" class="textinput1" required value="<?php echo $query2['password']; ?>"></td>
    </tr>
    <tr>
        <td>EMail</td>
        <td><input type="email" name="email" placeholder="Email" class="textinput1" value="<?php echo $query2['email']; ?>"></td>
    </tr>
    <tr>
        <td>Mobile No</td>
        <td><input type="text" maxlength="10" name="mob" placeholder="Mobile No" class="textinput1" onkeypress="return isNumber(event)" value="<?php echo $query2['mob']; ?>"></td>
    </tr>
    <tr>
        <td>Enrol No</td>
        <td><input type="text" maxlength="10" name="enr_no" placeholder="Enrolment No" class="textinput1" required value="<?php echo $query2['enr_no']; ?>" onkeypress="return isNumber(event)" ></td>
    </tr>
    <tr>
        <td>
            Gender
